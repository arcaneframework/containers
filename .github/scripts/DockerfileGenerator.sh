...
f_help() {
  ...
  echo "                            (ex: -a debug)"
  echo "  -d, --base_image_date     choose the date of the base image (default: stable)"
  echo "                            (ex: -d 20221230)"
...
# Si l'utilisateur n'a pas spécifié de date, on met 'stable'.
if [ -z "${BASE_IMAGE_DATE}" ]
then
  BASE_IMAGE_DATE='stable'
fi
...
then
  C_COMPILER='gcc'
  CXX_COMPILER='g++'
  CMAKE_CONFIG+='-D ARCANE_ACCELERATOR_MODE=CUDA -D CMAKE_CUDA_COMPILER=nvcc -D CMAKE_CUDA_ARCHITECTURES="80;90" '
...
echo ""
echo "IMAGE_BASE=         ${OS}:${COMPILER_NAME}${COMPILER_VERSION_WITH_DASH}_${IMAGE_VERSION}_${BASE_IMAGE_DATE}"
echo "DOCKERFILE.IN=      ${DFIN}"
echo "DOCKERFILE=         ${DFOUT}"
echo ""
echo "IMAGE_OUT=          arcane_${OS}:${IMAGE_VERSION}_${BUILD_TYPE}_latest"
